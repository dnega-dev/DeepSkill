...
|---|---|---|
| [**deepwiki-harvester**](./skills/deepwiki-harvester/) | The engine behind this repo — harvest any public repo's wiki via DeepWiki's free MCP endpoint, then run parallel multi-repo campaigns. Uses the DeepWiki MCP tools available in the host agent. | the method itself |
...
**Harvest your own:** use the [`deepwiki-harvester`](./skills/deepwiki-harvester/) skill with a host that has DeepWiki MCP configured. The skill defines the provenance, brief, synthesis, and verification contract without assuming one host-specific transport command.
